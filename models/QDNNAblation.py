...
class QDNNAblation(nn.Module):
    
    def __init__(self,opt):    
        super(QDNNAblation, self).__init__()
        
        self.input_dims = opt.input_dims
        self.output_dim = opt.output_dim
        self.device = opt.device
        self.num_modalities = len(self.input_dims)   
        self.measurement_enabled = opt.measurement_enabled
        self.modality_weighting = opt.modality_weighting
        self.last_time_stamp = opt.last_time_stamp
        ...
        self.hidden_dims = [self.text_out_dim]+self.hidden_dims[1:]
...
#            self.tensor_size = self.tensor_size *(d+1) 
        ...
        self.phase_embed = nn.ModuleList([PhaseEmbedding(opt.lookup_table.shape[0], hidden_dim,freeze = opt.phase_freeze) for hidden_dim in self.hidden_dims])
        ...
        if self.measurement_enabled:
            self.nn_input = self.num_measurements
        else:
            self.nn_input = self.tensor_size
            
        if self.output_dim == 1:
            self.fc_out = nn.Sequential(nn.Dropout(self.post_fusion_dropout_prob),
                                        nn.Linear(self.nn_input, self.post_fusion_dim),
                                        nn.ReLU(),
...
#        tensor_products = []
        ...
        if not self.modality_weighting:
...
#            tensor_product = hidden_units[0][:,i,:]
            tensor_product_real, tensor_product_imag = hidden_units_real[0][:,i,:], hidden_units_imag[0][:,i,:]
#            print(tensor_products_real.shape)
#            print(tensor_products_imag.shape)
...
#                h_added = h[:,i,:]
            ...
            if not self.measurement_enabled:
                tensor_product_real = tensor_product_real* w_i.expand_as(tensor_product_real)
                tensor_product_imag = tensor_product_imag* w_i.expand_as(tensor_product_imag)
        ...
        elif self.last_time_stamp:
            output = real_tensors[0]
        else:
            output = sum(real_tensors)
...
if __name__=="__main__":
    ...
    fmodel=QDNNAblation([40,12,20],100)
